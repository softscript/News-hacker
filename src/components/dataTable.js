...
const tableData = (props) => {
    ...
    let dataRows = _.compact(rows.map((row, index) => {
        if (!hiddenNewsIds.includes(row.objectID)) {
            return (
                <TableRow key={index}>
                    <TableCell>{index + 1}</TableCell>
                    <TableCell align="left">{row.num_comments}</TableCell>
                    <TableCell align="left">{props.voteCountObj[row.objectID] || 0}</TableCell>
                    <TableCell align="left" >
                        <img className='clickable'
                            onClick={() => props.handleUpVote(row.objectID)}
                            src={upVoteIcon} alt='bb'></img>
                    </TableCell>
                    <TableCell align="left">
                        <NewsDetails rowData={row} handleHideNews={props.handleHideNews} />
                    </TableCell>
                </TableRow>
            )
        } else {
